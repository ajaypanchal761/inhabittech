...
function Services() {
  ...
  const renderIcon = (service) => {
    if (service?.icon?.url) {
      ...
      return <img src={service.icon.url} alt={service.title} className="w-5 h-5 object-contain" />;
    } else if (service?.icon && typeof service.icon === 'string') {
      // Legacy support: if icon is SVG code, render it
      try {
        return <div dangerouslySetInnerHTML={{ __html: service.icon }} className="w-5 h-5" />;
      } catch {
  ...
  // Render large icon for service detail
  const renderLargeIcon = (service) => {
    if (service?.icon?.url) {
      // If icon is from Cloudinary, display as image
      return <img src={service.icon.url} alt={service.title} className="w-8 h-8 object-contain" />;
    } else if (service?.icon && typeof service.icon === 'string') {
      // Legacy support: if icon is SVG code, render it
      try {
        return <div dangerouslySetInnerHTML={{ __html: service.icon }} className="w-8 h-8 text-white" />;
      } catch {
        // Fallback
      }
    }
